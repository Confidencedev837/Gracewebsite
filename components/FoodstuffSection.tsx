...
const GalleryModal: React.FC<{ images: string[]; onClose: () => void }> = ({ images, onClose }) => {
  ...
  const prevSlide = () => setIndex((prev) => (prev > 0 ? prev - 1 : images.length - 1));
    ...
    >
      ...
      <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
        <AnimatePresence mode="wait" initial={false}>
          ...
          >
            {/* Simple check for video files by extension if possible, or just treat as image */}
            {images[index].toLowerCase().match(/\.(mp4|webm|ogg)(\?.*)?$/) ? (
              <video 
                src={images[index]} 
                controls 
                autoPlay 
  ...
  return (
    <section id="foodstuff" className="py-24 bg-[#0a0a14] relative overflow-hidden">
          ...
          <>
            ...
            >
              {filteredItems.map((item) => (
                ...
                >
                  <div className="relative aspect-video overflow-hidden cursor-pointer" onClick={() => setGalleryImages(item.images)}>
                    {item.images && item.images.length > 0 ? (
                      ...
                      <div className="w-full h-full bg-white/5 flex items-center justify-center text-gray-600">No Image Available</div>
                    )}
      ...
      <AnimatePresence>
        ...
        {galleryImages && <GalleryModal images={galleryImages} onClose={() => setGalleryImages(null)} />}
      </AnimatePresence>
    </section>
  );
};
