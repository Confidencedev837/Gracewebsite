...
export default function Admin() {
    ...
    const handleAddItem = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            await saveProduct({ ...newItem, imageFiles, videoFiles });
    ...
    const handleDelete = async (item: FoodItem) => {
        ...
        try {
            ...
            await deleteProduct(item.id, item.imageIds || [], item.videoIds || []);
                        ...
                        <div className="bg-[#0a0a14] p-8 rounded-3xl border border-white/5 sticky top-32">
                            ...
                            <form onSubmit={handleAddItem} className="space-y-4">
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative border-2 border-dashed border-white/10 rounded-xl p-4 text-center hover:border-blue-500/50 transition-all cursor-pointer">
                                            <input multiple type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => setImageFiles(Array.from(e.target.files || []))} />
                                            <Upload className="w-8 h-8 mx-auto text-gray-600 mb-2" />
                                            <span className="text-xs text-gray-500">{imageFiles.length > 0 ? `${imageFiles.length} images selected` : 'Click to add images'}</span>
                                        </div>
                                    </div>

                                    <div className="space-y-2">
                                        <label className="block text-sm text-gray-500 font-medium">Upload Videos (Optional)</label>
                                        <div className="relative border-2 border-dashed border-white/10 rounded-xl p-4 text-center hover:border-blue-500/50 transition-all cursor-pointer">
                                            <input multiple type="file" accept="video/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => setVideoFiles(Array.from(e.target.files || []))} />
                                            <Upload className="w-8 h-8 mx-auto text-blue-500/50 mb-2" />
                                            <span className="text-xs text-gray-500">{videoFiles.length > 0 ? `${videoFiles.length} videos selected` : 'Click to add videos'}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <button disabled={submitting} type="submit" className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl flex items-center justify-center space-x-2 disabled:opacity-50">
                                    ...
                                    <span>{submitting ? 'Uploading Product...' : 'Save Product'}</span>
                                </button>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {items.filter(i => i.name.toLowerCase().includes(search.toLowerCase())).map((item) => (
                                        <div key={item.id} className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden group">
                                            <div className="h-40 relative bg-[#05050a] flex items-center justify-center overflow-hidden">
                                                {item.images && item.images.length > 0 ? (
                                                    <Image src={item.images[0]} alt={item.name} fill className="object-cover" />
                                                ) : item.videos && item.videos.length > 0 ? (
                                                    <video src={item.videos[0]} className="w-full h-full object-cover" muted />
                                                ) : (
                                                    <div className="w-full h-full bg-white/5 flex items-center justify-center text-gray-600 italic">No Media</div>
                                                )}
                                                <div className="absolute top-2 left-2 flex gap-1">
                                                    {item.images && item.images.length > 0 && <span className="bg-blue-600 text-[8px] px-1.5 py-0.5 rounded text-white">{item.images.length}P</span>}
                                                    {item.videos && item.videos.length > 0 && <span className="bg-orange-600 text-[8px] px-1.5 py-0.5 rounded text-white">{item.videos.length}V</span>}
                                                </div>
